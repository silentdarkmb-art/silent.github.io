...
`,t.addEventListener("click",()=>{window.location.href=m+e.url}),o.appendChild(t)}),o.classList.add("show"))):l()}),i.addEventListener("keydown",function(e){var t;"Enter"===e.key&&(e.preventDefault(),(t=n(this.value)).length)&&(window.location.href=m+t[0].url),"Escape"===e.key&&l()}),document.addEventListener("click",function(e){var t=i.closest(".search-shell"),t=i.closest(".header-search")||t;t&&!t.contains(e.target)&&l()}))})();function setupDropdown(dropdownId,btnId){var drop=document.getElementById(dropdownId);var btn=document.getElementById(btnId);if(drop&&btn){let isOpen=!1;btn.addEventListener("click",e=>{e.preventDefault(),isOpen=!isOpen,drop.classList.toggle("open",isOpen)});drop.addEventListener("mouseenter",()=>{isOpen||drop.classList.add("open")});drop.addEventListener("mouseleave",()=>{isOpen||drop.classList.remove("open")});document.addEventListener("click",e=>{drop.contains(e.target)||(isOpen=!1,drop.classList.remove("open"))})}}setupDropdown("codesDropdown","codesBtn");setupDropdown("guidesDropdown","guidesBtn");setupDropdown("tipsDropdown","tipsBtn");setupDropdown("tutorialDropdown","tutorialBtn");(async()=>{let a=document.getElementById("moreGrid");if(a){var e=await g();let t=(location.pathname.split("/").pop()||"").toLowerCase();var n=[...e.filter(e=>e.url.toLowerCase()!==t)];for(let e=n.length-1;0<e;e--){var i=Math.floor(Math.random()*(e+1));[n[e],n[i]]=[n[i],n[e]]}a.innerHTML="",n.slice(0,6).forEach(e=>{var t=document.createElement("a");t.className="more-card",t.href=m+e.url,t.innerHTML=`
